...
public:
    ...
    void saveAllData(const std::vector<Product>& products, const std::vector<Invoice>& invoices);
    void appendInvoiceToLog(const Invoice& invoice);
...
private:
    ...
    void saveInvoices(const std::vector<Invoice>& invoices);
    void loadOrderItems(std::vector<Invoice>& invoices, const std::vector<Product>& allProducts);
    void saveOrderItems(const std::vector<Invoice>& invoices);
    std::string getInvoiceStringForLog(const Invoice& invoice);
};
#endif