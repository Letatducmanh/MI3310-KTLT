...
void ConsoleUI::handleImportProducts() {
    ...
    if (result.importedCount > 0) {
        service.saveProductsData(); // <<<--- GỌI LƯU FILE
    }
...
void ConsoleUI::handleRevenueByDate() {
    ...
    Logger::getInstance().log("Thuc hien bao cao doanh thu theo ngay: " + queryDate);
    // Gọi hàm service mới để lấy dữ liệu thống kê
    DailyStatistics stats = service.getStatisticsForDate(queryDate);

    clearScreen();
    std::cout << "\n=== THONG KE DOANH THU NGAY " << queryDate << " ===" << std::endl;
    ...
    if (stats.invoiceCount == 0) {
        ...
        std::cout << std::left << std::setw(20) << queryDate
