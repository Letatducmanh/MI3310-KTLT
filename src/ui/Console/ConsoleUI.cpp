...
void ConsoleUI::handleCreateInvoice() {
    ...
    std::cout << "Nhap ngay lap (dd/mm/yyyy): ";
    std::string date_input; std::getline(std::cin, date_input);
...
void ConsoleUI::handleRevenueByMonth() {
    ...
    if (sscanf(input.c_str(), "%d/%d", &month, &year) != 2 || month < 1 || month > 12) {
        std::cout << "Dinh dang khong hop le." << std::endl;
        return;
    }
...
void ConsoleUI::handleProductRevenueByMonth() {
    std::cout << "Nhap thang va nam de thong ke (mm/yyyy): ";
    std::string input; 
    std::getline(std::cin, input);
